...
int BGround_::start()
	...
	for (int ii = 0; ii < sprites.size(); ii++)
		{
		if(sprites[ii]->rot == 0)
		for (int xx = 0; xx < sprites[ii]->xlen;xx++)
			for (int yy = 0; yy < sprites[ii]->ylen; yy++)
				{
				...
				Img[tx * 3 + ty*Width * 3 + 0] = sprites[ii]->data[xx * 3 + yy*sprites[ii]->xlen * 3 + 0];
				Img[tx * 3 + ty*Width * 3 + 1] = sprites[ii]->data[xx * 3 + yy*sprites[ii]->xlen * 3 + 1];
				Img[tx * 3 + ty*Width * 3 + 2] = sprites[ii]->data[xx * 3 + yy*sprites[ii]->xlen * 3 + 2];
				}
		if (sprites[ii]->rot == 1)
		for (int xx = 0; xx < sprites[ii]->xlen; xx++)
				for (int yy = 0; yy < sprites[ii]->ylen; yy++)
				{
					...
					Img[tx * 3 + ty*Width * 3 + 0] = sprites[ii]->data[(sprites[ii]->xlen-xx-1) * 3 + yy*sprites[ii]->xlen * 3 + 0];
					Img[tx * 3 + ty*Width * 3 + 1] = sprites[ii]->data[(sprites[ii]->xlen - xx - 1) * 3 + yy*sprites[ii]->xlen * 3 + 1];
					Img[tx * 3 + ty*Width * 3 + 2] = sprites[ii]->data[(sprites[ii]->xlen-xx-1) * 3 + yy*sprites[ii]->xlen * 3 + 2];
				}
		}
	return 1;
	}
